...
void read_write_lock_init(struct ReadWriteLock* rw)
{
	...
	rw->n_reading = 0, rw->n_writing = 0;
	rw->n_readers_waiting = 0, rw->n_writers_waiting = 0;
	rw->signalling = true; // reading allowed
}
...
void before_write(struct ReadWriteLock* rw)
{
	...
	// someone is in the library or there are readers waiting and its their turn
	while ((rw->n_reading > 0 || rw->n_writing > 0) || (rw->n_readers_waiting > 0 && rw->signalling)) {
...
void before_read(struct ReadWriteLock* rw)
{
	...
	// someone is writing or its writers turn
	if (rw->n_writing > 0 || !rw->signalling) {
