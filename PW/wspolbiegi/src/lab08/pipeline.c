    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (!pid) {
            ...
            if (i < argc - 1) {
				...
				ASSERT_SYS_OK(close(pipe_fd[0]));
				ASSERT_SYS_OK(dup2(pipe_fd[1], STDOUT_FILENO));
				ASSERT_SYS_OK(close(pipe_fd[1]));

                int _placeholder = pipe_fd[1];
